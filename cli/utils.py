...
def shell_run(*args, cwd: Union[str, pathlib.Path] = None, shell=True, tty=False, env: Optional[dict] = None) -> str:
    if cwd is None:
        cwd = ROOT.as_posix()
    ...
    if tty:
        ...
        proc = subprocess.Popen(cmd, stdout=pipe, stderr=pipe, stdin=sys.stdin, cwd=cwd, shell=True, bufsize=1, env=env)
        ...
        return subprocess.check_output(cmd, cwd=cwd, shell=shell, env=env).decode('utf-8')
